...
void show_pattern(color_pattern_t pattern)
{
    ...
    static uint8_t s_chBuffer = 0;
    s_chBuffer = (s_chBuffer + 1) & 0x01;
    
#if(DISPLAY_BITS_PER_PIXEL_INPUT0==32)
    uint32_t * p = (uint32_t *)&fb_background[s_chBuffer][0];
#else
    uint16_t * p = (uint16_t *)&fb_background[s_chBuffer][0];
#endif
